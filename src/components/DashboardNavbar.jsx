...
const DashboardNavbar = ({ profileImage }) => {
  ...
  const coins = 0;
  const life = 0;
  const navItems = ["Home", "Library", "Challenges", "Shop"];

  useEffect(() => {
    if (profileImage) setSavedImage(profileImage);
    else {
  ...
  return (
    <nav
      ...
      style={{
        boxShadow: "56px 0 15px -5px rgba(0,0,0,0.4), 0 4px 6px -2px rgba(0,0,0,0.2)",
      }}
    >
      ...
      <div className="hidden lg:flex items-center font-medium space-x-20">
        {navItems.map((item) => (
          <button
            ...
            className={`text-base lg:text-lg pb-1 border-b-2 transform transition-all duration-200 ${
              activeItem === item
                ? "border-white"
      ...
      <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
        <div className="flex items-center space-x-2">
          <img src="/src/assets/life.png" alt="Life Icon" className="h-6 w-6" />
          <span className="text-base font-medium">{life}</span>
        </div>
      ...
      <div className="flex lg:hidden items-center space-x-3 sm:space-x-4">
        <div className="flex items-center space-x-1 sm:space-x-2">
          <img src="/src/assets/life.png" alt="Life" className="h-5 w-5" />
        ...
        <div className="absolute top-[60px] right-0 lg:hidden bg-[#870022] w-[60%] sm:w-[50%] p-5 rounded-bl-xl shadow-lg flex flex-col items-center space-y-3">
          ...
          {navItems.map((item, index) => (
            <React.Fragment key={item}>
              <button
                onClick={() => handleNavClick(item)}
                className="text-base sm:text-lg py-2 w-full text-white"
              >
                {item}
              </button>
